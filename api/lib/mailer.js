 ...
 * 출처: 기존 판매 시스템 confirm-payment.js / send-download.js 의 메일 부분을 콘솔 시스템 문안으로 교체
 *
...
<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Malgun Gothic','Apple SD Gothic Neo',sans-serif;max-width:600px;margin:0 auto;padding:36px 20px;color:#1a2238;">
  <div style="text-align:center;margin-bottom:28px;">
    ...
    <h1 style="font-size:22px;margin:16px 0 6px;">${name}님, 콘솔 시스템 ${p.renewal ? '다운로드 링크를 다시 보내드립니다' : '구매해 주셔서 감사합니다'}</h1>
  ...
  <div style="background:#1A2238;color:#fff;padding:26px;border-radius:16px;margin-bottom:22px;text-align:center;">
    ...
    <a href="${esc(p.downloadLink)}" style="display:inline-block;background:#C9A961;color:#1A2238;padding:14px 34px;text-decoration:none;border-radius:10px;font-weight:800;font-size:17px;">콘솔시스템 APK 다운로드</a>
  ...
  <div style="font-size:12px;color:#8A8F9E;line-height:1.7;border-top:1px solid #EAEAEC;padding-top:14px;">
    이 앱은 투자 자문·일임 서비스가 아니며, 동봉된 샘플 전략은 교육·출발점용입니다. 수익을 보장하지 않으며 모든 매매 판단과 결과는 사용자 본인의 책임입니다.<br>
...
async function sendPurchaseEmail(p) {
    const t = transporter();
    await t.sendMail({
        from: `"콘솔 시스템 (WAAT)" <${process.env.GMAIL_USER}>`,
        to: p.email,
        subject: p.renewal
            ? `[콘솔 시스템] 다운로드 링크 재발급 — 주문 ${p.orderId}`
            : `[콘솔 시스템] 구매 완료 — 다운로드 링크와 라이선스 키 (주문 ${p.orderId})`,
