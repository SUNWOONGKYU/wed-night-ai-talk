...
너는 사용자가 만든 콘솔 폴더(현재 작업 디렉토리 = `PROJECT`)에서 실행 중이다. 이 폴더가 곧 **기본 프로젝트 '트레이딩 시그널'** 이 된다(주식 스캐너·샘플 전략·전략 빌더 동봉). 다른 프로젝트는 설치 후 폰 앱의 `[프로젝트 ▾] → 프로젝트 추가` 로 늘린다. 아래 단계를 **순서대로** 수행하고, 각 단계 결과를 한 줄씩 보고하라. 실패한 단계는 원인과 사용자가 직접 할 일을 정확히 알려주고 멈춰라. 추측으로 넘어가지 마라.
...
**3B. 기본 프로젝트 '트레이딩 시그널' 엔진** (`PROJECT\scanner_web.py` 가 이미 있으면 건너뜀)
1. 내려받아 `PROJECT` 에 푼다(기존 파일은 덮어쓰지 않는다):
```powershell
...
```
   풀린 폴더 안에서 `scanner_web.py` 가 있는 폴더의 내용을 `PROJECT\` 로 복사(`_mobile_remote` 는 건드리지 않는다).
...
4. `projects`: `[ { "name": "트레이딩 시그널", "dir": "<PROJECT 절대경로>" } ]` — 경로의 `\` 는 JSON 에서 `\\`. (프로젝트 이름은 폴더 이름이 아니라 **'트레이딩 시그널'** 로 고정)
