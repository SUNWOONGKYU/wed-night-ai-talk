...
(async () => {
    ...
    const info = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('.footer-related a')).map(a => ({
            ...
            desc: a.querySelector('.related-desc')?.textContent.trim()
        }));
        return links;
    });
    console.log(JSON.stringify(info, null, 2));
    await page.screenshot({ path: path.join('tmp', 'footer_3sites_desktop.png'), fullPage: false });
    await page.setViewport({ width: 390, height: 844 });
    await page.goto('https://waat.community/?_=' + Date.now(), { waitUntil: 'networkidle2', timeout: 60000 });
    await page.evaluate(() => document.querySelector('footer').scrollIntoView({ behavior: 'instant', block: 'end' }));
    await new Promise(r => setTimeout(r, 400));
    await page.screenshot({ path: path.join('tmp', 'footer_3sites_mobile.png'), fullPage: false });
    await browser.close();
