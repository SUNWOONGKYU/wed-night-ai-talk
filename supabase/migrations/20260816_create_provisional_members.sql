...
-- 이메일로 빠른 조회를 위한 인덱스
CREATE INDEX IF NOT EXISTS idx_provisional_members_email ON provisional_members(email);
CREATE INDEX IF NOT EXISTS idx_provisional_members_submitted ON provisional_members(submitted_at);

-- 권한 설정 (RLS)
ALTER TABLE provisional_members ENABLE ROW LEVEL SECURITY;

-- 정책 1: 누구나 조회 가능 (이메일로만 확인하도록)
DROP POLICY IF EXISTS "예비 멤버 이메일 확인 가능" ON provisional_members;
...
-- 정책 2: 관리자만 삽입 가능
DROP POLICY IF EXISTS "관리자만 예비 멤버 등록" ON provisional_members;
...
-- 정책 3: 관리자만 수정 가능
DROP POLICY IF EXISTS "관리자만 예비 멤버 수정" ON provisional_members;
