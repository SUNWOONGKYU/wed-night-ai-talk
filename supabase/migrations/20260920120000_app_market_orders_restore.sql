-- ===========================================================================
-- 앱 마켓 (콘솔 시스템 판매) — Supabase 주문·설정 복원 (2026-09-20 15:00)
--
-- 경위: 100000(생성) → 110000(철회: 결제를 Google Sheets 로 옮기려던 지시) → PO 최종 결정으로
--       Supabase 구현 유지 + Google Sheets 는 '미러링'(api/market/sheet-sync.js)만 한다.
--       110000 이 이미 원격에서 DROP 됐으므로 100000 과 같은 내용을 다시 만들고, 아래를 더한다:
--   · app_settings 초기값: PO 확인 완료 — 기존 판매 시스템의 계좌·카카오페이 링크 재사용, 판매자 표기 확정
--   · 결제 수단은 bank / kakaopay 2개만 (PG 미도입 — PO 결정)
--   · 시트 미러링용 Database Webhook 트리거 (supabase_functions.http_request) — 미활성 프로젝트면 NOTICE 만
-- ===========================================================================
...
-- 초기값 — PO 확인 완료(2026-09-20): 기존 판매 시스템의 계좌·카카오페이 영구 링크를 그대로 쓴다.
-- (이 값들은 기존 sales-system/index.html 에 공개돼 있던 값이다.) 관리자 화면에서 언제든 바꿀 수 있다.
INSERT INTO public.app_settings (key, value) VALUES
    ('bank_name',       '하나은행'),
    ('bank_account',    '287-910921-40507'),
    ('bank_holder',     '선웅규'),
    ('kakaopay_link',   'https://qr.kakaopay.com/Ej8qUBxLx138303594'),
    ('download_url',    ''),      -- 결제 확인 후에만 내려주는 APK 주소 (공개 RPC 에 절대 안 나감) — 배포 시 관리자가 입력
    ('product_price',   '9900'),  -- 원, VAT 포함
    ('product_version', 'v1.0'),
    ('seller_info',     E'판매자: 선웅규 (공인회계사 · AI 마스터)
소개: https://saa.house/about
Facebook: https://www.facebook.com/tlc500
문의: wksun999@hanmail.net
사업자등록번호: [입력 예정] · 통신판매업신고: [입력 예정]')
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value WHERE public.app_settings.value = '';
...
-- ---------------------------------------------------------------------------
-- 6) Google Sheets 미러링 — Database Webhook
--    주문 INSERT/UPDATE 마다 https://www.waat.community/api/market/sheet-sync 로 POST.
--    Vercel 함수는 받은 record 를 믿지 않고 get_app_order(order_no, contact) 로 다시 확인한 뒤
--    Orders 시트에 upsert 한다 (비밀 헤더 불필요). 클라이언트도 같은 함수를 호출하므로 웹훅은 보강용.
--    supabase_functions 스키마(대시보드 Database → Webhooks 활성화 시 생성)가 없으면 건너뛴다.
-- ---------------------------------------------------------------------------
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'supabase_functions') THEN
        EXECUTE 'DROP TRIGGER IF EXISTS app_orders_sheet_sync_trg ON public.app_orders';
        EXECUTE $t$
            CREATE TRIGGER app_orders_sheet_sync_trg
            AFTER INSERT OR UPDATE ON public.app_orders
            FOR EACH ROW
            EXECUTE FUNCTION supabase_functions.http_request(
                'https://www.waat.community/api/market/sheet-sync',
                'POST',
        ...
        $t$;
        RAISE NOTICE '[ok] webhook trigger app_orders_sheet_sync_trg';
    ELSE
        RAISE NOTICE '[skip] supabase_functions 스키마 없음 — 대시보드에서 Database Webhooks 를 켠 뒤 이 블록을 다시 실행';
    END IF;
END $$;
